...
| A4 | I | ¿Qué cuenta de abono (`accountCredit`) válida debemos usar en certificación? ¿Hay cuentas de prueba pre-creadas? | El banco **envía un usuario de pruebas con una cuenta** para el testeo. | **Cerrada el 2026-09-20 (Andres): el banco no tiene cuenta de pruebas.** Se ensaya en producción con pagos propios de monto mínimo (ESTADO, decisión 21). B0 queda limitado al login y el cifrado. |
...
| C5 | I | `cancelQR` sobre un QR ya vencido o ya anulado: ¿retorna error o éxito idempotente? ¿Qué `responseCode` en cada caso? | **Se puede anular un QR vencido.** No dice qué pasa con uno ya anulado. | Anular al vencer es posible. **Observado en producción (2026-09-14):** la doble anulación **no** es idempotente. Devuelve `responseCode 403`, el mismo código que al anular un QR pagado. El adaptador consulta `statusQR` ante ese rechazo y lo trata como éxito solo si el QR figura anulado (`02-hallazgos-produccion.md` §3.1). |
...
| C9 | D | ¿Se aplican **comisiones** al comercio receptor por cobro QR acreditado? ¿Cómo se liquidan (descuento en el abono o cargo aparte)? | Se **negocian con el ejecutivo de negocios**. | **Cerrada el 2026-09-17 por el dueño: no hay comisión; el servicio del banco es gratuito.** El monto acreditado es el del QR, así que la conciliación por monto exacto no necesita tolerancia por comisión (ESTADO, decisión 17). |
