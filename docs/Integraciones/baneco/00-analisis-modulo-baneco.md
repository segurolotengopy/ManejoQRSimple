...
| R8 | QR pagable en el banco después de que el cobro lo abandonó (vencido en nuestro reloj, renovado o anulado): `dueDate` es un día, no un instante (C4) | Dinero acreditado sin registrarse; conciliación rota | **Mitigado (2026-09-12).** La máquina de estados exige la constancia de `cancelQR` para soltar un QR pagable (vencer, anular, ventana agotada); se consulta el banco antes de vencer; un pago que igual llegue sobre un QR vencido va a `EN_REVISION` (`ABONO_TARDIO`). Queda la carrera de segundos entre la última consulta y la anulación: la cubre el cierre diario. |
| R9 | `transactionId` sin unicidad del lado del banco (C3): un `generateQR` reintentado tras un timeout puede dejar un QR pagable que no conocemos | Igual que R8 | **Mitigado.** El cliente no reintenta `generateQR` por su cuenta; el satélite corre el cierre diario (`paidQR` del día anterior) y reporta el abono sin cobro como huérfano. Los huérfanos hoy van al log del satélite: persistirlos para la consola es una mejora pendiente. |
